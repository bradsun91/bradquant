...
class RiskMetrics(object):
    ...
    def alpha_beta(strategy_ret, benchmark_ret, benchmark_str, show_all):
        ...
        if show_all == False:
            print ('Gross Strategy Alpha: {:0.04f}'.format(alpha))
            print ('Gross Strategy Beta: {:0.04f}'.format(beta))
        else:
            print ('Gross Strategy Alpha: {:0.04f}'.format(alpha))
            print ('Gross Strategy Beta: {:0.04f}'.format(beta))
            print (model.summary())
    ...
    def net_alpha_beta(net_each_return_pct, benchmark_ret, benchmark_str, show_all):
        ...
        # otherwise it will only return alpha and beta.
        if show_all == False:
            print ('Net Strategy Alpha: {:0.04f}'.format(alpha))
            print ('Net Strategy Beta: {:0.04f}'.format(beta))
        else:
            print ('Net Strategy Alpha: {:0.04f}'.format(alpha))
            print ('Net Strategy Beta: {:0.04f}'.format(beta))
            print (model.summary())
...
##### Gross rolling sharpe plotting & Net rolling sharpe plotting #####
        ...
        annual_rolling_std.plot(ax = ax9, color = 'purple', lw = 1)
        ax9.set_title('Annual Rolling Volatility')

        # plot gross rolling sharpe
        rolling_sharpe.plot(ax = ax10, color = 'black', lw = 1)
        ax10.set_title('Rolling Sharpe')

        # plot net annual rolling volatility
        net_annual_rolling_std.plot(ax = ax11, color = 'purple', lw = 1)
        ax11.set_title('Net Annual Rolling Volatility')

        # plot net rolling sharpe
        net_rolling_sharpe.plot(ax = ax12, color = 'purple', lw = 1)
        ax12.set_title('Net Rolling Sharpe')


        self.core_bt_df2_bm.commission_fees_impact_level.plot(ax = ax13, color = 'black', lw = 1)
        ax13.set_title('Commission Fees Impact Level Risk Monitor')

        self.ticker_qty.plot(ax = ax14, color = 'teal', lw = 1)
        ax14.set_title('Position Monitor')
        ...
        # Gross Stats
        print ("Gross Alpha Beta Stats: ", RiskMetrics.alpha_beta(self.strategy_ret, self.benchmark_ret, self.benchmark_str, self.show_all))
        ...
        # Net Stats
        print ("Net Alpha Beta Stats: ", RiskMetrics.net_alpha_beta(self.net_each_return_pct, self.benchmark_ret, self.benchmark_str, self.show_all))
        ...
        print ("Net Max Drawdown: {:0.02f}".format(RiskMetrics.net_max_drawdown(self.net_cum_return_pct, self.how)))
        ...
        print ("Net Annual Volatility: {:0.02f}%".format(100*RiskMetrics.net_annual_volatility(self.net_each_return_pct, self.freq_multiplier)))
