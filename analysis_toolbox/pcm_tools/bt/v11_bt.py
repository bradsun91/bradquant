...
"""
In this Version 11, main updates:

1. Portfolio position control (no cum return should be less than 0): being added as mentioned in the version 10 bt documentation.
2. In this version, a lower commission function will be replaced instead.
3. Re-organize how the profit dataframe is generated row by row
4. Added trunks function for splitting large files into different trunks
""" 
def rows_per_trunk(df, trunks_needed):
    rows_per_file = len(df)/(trunks_needed+1)  
    return math.ceil(rows_per_file)
...
def clean_extracted_from_df(dataframe, one_ticker, ticker_with_col_str_final):
    

    #### 4.1.2.1  Delete the UTC and space in the date strings 
    str_dt_list = list(dataframe.date)
    for i, s in enumerate(str_dt_list):
        str_dt_list[i] = str_dt_list[i].replace(" UTC", "")
    ...
    for i, s in enumerate(str_dt_list):
        str_dt_list[i] = datetime.strptime(str_dt_list[i],'%Y-%m-%d %H:%M:%S')
    dataframe.index = str_dt_list
    del dataframe['date']

    #### 4.1.3  Select specific tickers from the sample dataframe
    ticker_filtered = dataframe[(dataframe.tickers == one_ticker)]
...
# Build a function to directly output the different percentiles thresholds: 
...
def ib_equity_commissions_lower(quantity, fill_price):
    """
    """
    commission = min(0.0025 * fill_price * quantity, max(0.1, 0.0025 * quantity)) 
    return commission
            ...
            for i, (ts, signl) in enumerate(signal_updates.items()):            
                ...
                if signl != init_signal:   #signal starts to change, meaning we need to enter or exit positions
                    ...
                    if signl == enter_signal:  #only need to update position's qty here
                        ...
                        each_commission_cost = ib_equity_commissions_lower(abs(init_ticker_qty), ticker_price_updates[ts])
                        ...
                        filled_price_impact_chg_dlr = filled_price_impact_abs if strategy_drct == 'L' else (-filled_price_impact_abs if strategy_drct == 'S' else 0) 
                        actual_filled_price = filled_price_impact_chg_dlr + init_ticker_price
                        each_commission_cost = ib_equity_commissions_lower(abs(updated_qty), ticker_price_updates[ts])
                        cum_commissions += each_commission_cost                        
                        init_ticker_qty = 0 
...
#             df2['each_profit'] = data_df['profit']
            ...
            # for i, item in enumerate(df2['net_cum_profit']):
            #     if df2['net_cum_profit'][i] <= 0:
            #         df2['net_cum_profit'][i] = 0
...
##### Calculate prepping for net strategy's Alpha, not Jensen's constant #####
        ...
        self.core_bt_df2_bm.commission_fees_impact_level.plot(ax = ax11, color = 'black', lw = 1)
        ax11.set_title('Commission Fees Impact Level Risk Monitor', fontsize = 16)
        ax11.set_xlabel('none').set_visible(False)

        self.ticker_qty.plot(ax = ax12, color = 'teal', lw = 1)
        ax12.set_title('Position Monitor', fontsize = 16)
        ax12.set_xlabel('none').set_visible(False)

        RiskMetrics.underlying_pricing_risk_monitor(self.ticker_price, self.rolling_window).plot(ax = ax13, color = 'red', lw = 1)
        ax13.set_title('Underlying Pricing Risk Monitor', fontsize = 16)
